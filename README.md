...
- 根据风险等级调用 DeepSeek API 生成不同风格的支持性回复
- 注入心理健康领域知识库作为 LLM System Prompt（`prompt/`）
- 对话历史持久化与多轮对话上下文注入
...
│              ▼                                   │
│  ┌─────────────────────────────────────────────┐ │
│  │  4. reply_step (LLM Service / DeepSeek)     │ │
│  │                                             │ │
...
│   │   ├── memory_service.py       # EMA 对话记忆平滑 + 对话历史持久化
│   │   ├── crisis_service.py       # 关键词安全检测 + risk_level 映射
│   │   └── llm_service.py          # DeepSeek LLM 回复生成（含知识库注入）
│   ├── prompt/
│   │   ├── knowledge.md            # 心理健康领域知识（YAML frontmatter + Markdown）
│   │   ├── system_prompt.md        # 回复生成的 System Prompt 模板
│   │   ├── knowledge_loader.py     # 知识库加载器（KnowledgeBase）
│   │   ├── loader_test.py          # knowledge_loader 测试
...
| 模型 | 输入 | 特征维度 | 路由条件 |
|------|------|----------|----------|
| V1 (纯文本) | 仅文本 | 8维语义特征（DeepSeek 结构化输出提取） | 仅传入文本 |
...
| `LLM_MODEL` | `qwen2.5:1.5b` | 回复模型名（响应中标识用） |
| `OLLAMA_BASE_URL` | `http://localhost:11434` | Ollama 服务地址（备用） |
| `API_KEY` | (无) | DeepSeek API Key（评分引擎 + 回复生成） |
| `BASE_URL` | (无) | DeepSeek API 地址（如 `https://api.deepseek.com`） |
| `LOG_LEVEL` | `INFO` | 日志级别 |
